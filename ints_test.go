...
func TestMapIntToString(t *testing.T) {
	...
	SendInts(intCh, 0, 1, 2, 3)

	// Act
	strCh := MapIntToString(intCh, toString)
...
func TestMergeIntChs(t *testing.T) {
	...
	SendInts(ch1, testValues[:3]...)
	SendInts(ch2, testValues[3:]...)

	// Act
	ch := MergeIntChs(ch1, ch2)

	// Assert
	actualValues := ToIntSlice(ch)
